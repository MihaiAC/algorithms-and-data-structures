...
class Solution:
    def calculateNrNodesEachSubtreeAndDistancesFromRoot(
        self, root: int, parent_node: int, dist_from_root: int
    ) -> int:
        nr_subtree_nodes = 0
        ...
        for neighbor in self.neighbors[root]:
            if neighbor == parent_node:
                ...
                nr_subtree_nodes += (
                    self.calculateNrNodesEachSubtreeAndDistancesFromRoot(
                        neighbor, root, dist_from_root + 1
                    )
                )
    ...
    def calculate_answer(self, root: int, parent_node: int):
        if root == 0:
            ...
            self.answer[root] = (
                self.answer[parent_node] - 2 * self.nr_nodes_subtree[root] + self.n - 2
            )
