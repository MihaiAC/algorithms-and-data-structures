...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def inner(node: Optional[TreeNode]) -> Tuple[int, int]:
            ...
            open_curr = maxIgnoreNone(
                node.val,
                sumIgnoreNone(node.val, open_left),
                sumIgnoreNone(node.val, open_right),
            )
            closed_curr = maxIgnoreNone(
                node.val,
                open_left,
                open_right,
                closed_left,
                closed_right,
                open_curr,
                sumIgnoreNone(node.val, open_left, open_right),
            )
