...
class Solution:
    @staticmethod
    def searchPreviousNonOverlappingWindow(
        startTime: int, endTime: List[int], sortedIndices: List[int], start_index: int
    ) -> int:
    ...
    def jobScheduling(
        ...
        for ii in range(len(sortedIndices)):
            ...
            jj = Solution.searchPreviousNonOverlappingWindow(
                startTime[sortedIndices[ii]], endTime, sortedIndices, ii - 1
            )
            found = jj >= 0
