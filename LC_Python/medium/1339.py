...
class Solution:
    ...
    def calculateMaxProduct(root: Optional[TreeNode], sum_ancestors: int) -> int:
        ...
        max_product_left = Solution.calculateMaxProduct(
            root.left, sum_ancestors + root.val[0] + root.val[2]
        )
        max_product_right = Solution.calculateMaxProduct(
            root.right, sum_ancestors + root.val[0] + root.val[1]
        )
