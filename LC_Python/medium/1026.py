...
class Solution:
    @staticmethod
    def maxAncestorDiffRec(
        root: Optional[TreeNode], min_ancestor: int, max_ancestor: int
    ):
        if root is None:
        ...
        return max(
            Solution.maxAncestorDiffRec(root.left, min_ancestor, max_ancestor),
            Solution.maxAncestorDiffRec(root.right, min_ancestor, max_ancestor),
        )
