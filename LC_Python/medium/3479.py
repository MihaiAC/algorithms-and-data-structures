...
class SegmentTree:
    ...
    def build(
        self, arr: List[int], curr_idx: int, segment_left: int, segment_right: int
    ):
        if segment_left == segment_right:
            ...
            self.tree[curr_idx] = max(
                self.tree[2 * curr_idx + 1], self.tree[2 * curr_idx + 2]
            )

    def find_first(
        self, curr_idx: int, segment_left: int, segment_right: int, x: int
    ) -> int:
