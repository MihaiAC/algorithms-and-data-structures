...
def find_max_nr_reflections(dimensions, signs, corners, your_position, distance):
    ...
    while True:
        if (
            sq_euclidean_dist(your_position, current_corners[0]) >= distance_sq
            and sq_euclidean_dist(your_position, current_corners[1]) >= distance_sq
        ):
...
def calculate_reflection_position(dimensions, initial_point, signs, ii, jj):
    ...
    if ii % 2 == 1:
        reflection_absolute_position[0] = (
            relative_origin_x + dimensions[0] - initial_point[0]
        )
    else:
        reflection_absolute_position[0] = relative_origin_x + initial_point[0]

    if jj % 2 == 1:
        reflection_absolute_position[1] = (
            relative_origin_y + dimensions[1] - initial_point[1]
        )
    else:
...
def count_quadrant_shots(
    ...
    for ii in range(1, max_refl[quadrant] + 1):
        for jj in range(1, max_refl[quadrant] + 1):
            refl_pos = calculate_reflection_position(
                dimensions, guard_position, signs[quadrant], ii, jj
            )
            ...
            if new_slope in slopes or (
                new_slope in your_reflections and your_reflections[new_slope] < aux_dist
            ):
                continue
...
def find_all_reflections_in_quadrant(
    dimensions, real_points, distance, signs, max_refl, quadrant
):
    max_distance = distance**2
    ...
    for ii in range(1, max_refl[quadrant] + 1):
        for jj in range(1, max_refl[quadrant] + 1):
            for point in real_points:
                refl_pos = calculate_reflection_position(
                    dimensions, point, signs[quadrant], ii, jj
                )
...
def count_OxOy_reflection_frames_shots(
    dimensions,
    your_position,
    guard_position,
    distance,
    ...
    for ii in range(1, max_refl_direction + 1):
        ...
        if Ox:
            refl_pos = calculate_reflection_position(
                dimensions, guard_position, sign, ii, 0
            )
        else:
            refl_pos = calculate_reflection_position(
                dimensions, guard_position, sign, 0, ii
            )
...
def solution(dimensions, your_position, guard_position, distance):
    ...
    max_refl = dict()
    for axe in axes:
        max_refl[axe] = find_max_nr_reflections(
            dimensions, axes_signs[axe], nswe_corners[axe], your_position, distance
        )
    ...
    for quadrant in quadrants:
        youcorner_reflections[quadrant] = find_all_reflections_in_quadrant(
            dimensions, positions, distance, quadrant_signs, max_refl, quadrant
        )
    ...
    if guard_position[0] != your_position[0]:
        count_up, slopes_up = count_OxOy_reflection_frames_shots(
            dimensions,
            your_position,
            guard_position,
            distance,
            ...
            (1, 1),
            False,
            total_yourcorner_reflections,
        )
        count_down, slopes_down = count_OxOy_reflection_frames_shots(
            dimensions,
            your_position,
            guard_position,
            distance,
    ...
    if guard_position[1] != your_position[1]:
        count_left, slopes_left = count_OxOy_reflection_frames_shots(
            dimensions,
            your_position,
            guard_position,
            distance,
        ...
        count_right, slopes_right = count_OxOy_reflection_frames_shots(
            dimensions,
            your_position,
            guard_position,
            distance,
    ...
    corners_slopes = dict()
    for quadrant in quadrants:
        corners_slopes[quadrant] = calculate_slope(
            your_position, first_corners[quadrant]
        )
    ...
    for quadrant in quadrants:
        ...
        total_count += count_quadrant_shots(
            slopes_init,
            dimensions,
            your_position,
            guard_position,
            ...
            max_refl,
            quadrant,
            youcorner_reflections[quadrant],
        )
