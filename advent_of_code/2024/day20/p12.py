...
class Solution:
    ...
    def compute_distance_to(self, coords: Tuple[int, int]) -> List[List[int]]:
        min_dist = [
            [float("inf") for _ in range(self.ncols)] for _ in range(self.nrows)
        ]
        curr_dist = 0
        queue = deque([coords])
        while len(queue) > 0:
            for _ in range(len(queue)):
                ...
                for dx, dy in self.DELTAS:
                    if (
                        0 <= cx + dx < self.nrows
                        and 0 <= cy + dy < self.ncols
                        and self.matrix[cx + dx][cy + dy] != "#"
                        and min_dist[cx + dx][cy + dy] == float("inf")
                    ):
    ...
    def calculate_n_shortcuts(self, threshold: int, max_cheat_dist: int) -> int:
        ...
        for ii in range(self.nrows):
            for jj in range(self.ncols):
                ...
                for dx in range(-max_cheat_dist, max_cheat_dist + 1):
                    for dy in range(
                        -(max_cheat_dist - abs(dx)), (max_cheat_dist - abs(dx) + 1)
                    ):
                        nx = ii + dx
                        ny = jj + dy
                        if (
                            0 <= nx < self.nrows
                            and 0 <= ny < self.ncols
                            and self.matrix[nx][ny] != "#"
                        ):
                            old_dist = dist_to_end[ii][jj]
                            new_dist = (
                                Solution.manhattan_dist((ii, jj), (nx, ny))
                                + dist_to_end[nx][ny]
                            )
        ...
        print(
            [
                (key, len(shortcut_len_count[key]))
                for key in sorted(shortcut_len_count.keys(), reverse=True)
            ]
        )
