    ...
    def calculate_and_print_lowest_safety_factors(self) -> None:
        ...
        for iteration in tqdm(range(1, self.MAXITER + 1)):
            for idx in range(self.N):
                self.x_coords[idx] = (
                    self.x_coords[idx] + self.velocities[idx][0]
                ) % self.X
                self.y_coords[idx] = (
                    self.y_coords[idx] + self.velocities[idx][1]
                ) % self.Y
            ...
            if len(max_heap) < self.HEAP_SIZE:
                heapq.heappush(
                    ...
                    (
                        ...
                        iteration,
                        deepcopy(self.x_coords),
                        deepcopy(self.y_coords),
                    ),
                )
            else:
        ...
        while len(max_heap) > 0:
            ...
            plt.savefig(
                "Iteration " + str(iteration) + " safety_factor=" + str(-safety_factor)
            )
            plt.clf()
