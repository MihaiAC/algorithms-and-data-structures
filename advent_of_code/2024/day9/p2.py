...
class Solution:
    ...
    def calculate_checksum(self) -> int:
        ...
        while len(non_empty_blocks_initial) > 0:
            ...
            empty_block_idx = empty_blocks.bisect_left(
                (non_empty_block_length, -float("inf"), -float("inf"))
            )
            if empty_block_idx == len(empty_blocks):
                non_empty_blocks.add(
                    (non_empty_block_idx, 0, non_empty_block_ID, non_empty_block_length)
                )
            else:
                ...
                if non_empty_block_idx > leftmost_idx:
                    ...
                    empty_block_length, empty_block_idx, empty_block_counter = (
                        value_with_leftmost_idx
                    )
                    non_empty_blocks.add(
                        (
                            empty_block_idx,
                    ...
                    if non_empty_block_length < empty_block_length:
                        empty_blocks.add(
                            (
                                empty_block_length - non_empty_block_length,
                                empty_block_idx,
        ...
        for (
            ...
            empty_block_idx,
            empty_block_counter,
        ) in empty_blocks.islice():
            non_empty_blocks.add(
                (empty_block_idx, empty_block_counter + 1, 0, empty_block_length)
            )
