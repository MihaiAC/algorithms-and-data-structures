...
class Solution:
    ...
    def init_tkinter(self):
        ...
        self.footer_counter = tk.Label(
            self.root,
            text="Moves: 0/" + str(self.total_moves),
            font=("Helvetica", 14),
            bg="black",
            fg="white",
        )
        self.footer_counter.grid(
            row=self.nrows, column=0, columnspan=self.ncols, pady=(10, 0)
        )
    ...
    def is_possible_move_box_vert(
        self, cx: int, cy: int, delta: Tuple[int, int]
    ) -> bool:
        dx, dy = delta
        ...
        if self.matrix[nx][ny] == "]" and self.is_possible_move_box_vert(
            nx, ny - 1, delta
        ):
            # self.matrix[rsnx][rsny] can be '.' or '['. It cannot be '#', ']' or '@'.
            if self.matrix[rsnx][rsny] == "." or (
                self.matrix[rsnx][rsny] == "["
                and self.is_possible_move_box_vert(rsnx, rsny, delta)
            ):
                return True
    ...
    def move_box_horiz_if_possible(
        self, cx: int, cy: int, delta: Tuple[int, int]
    ) -> bool:
        iy = cy
        dy = delta[1]
    ...
    def make_move_return_new_pos(
        ...
        if self.matrix[nx][ny] == "#":
            ...
            if dx == 0:
                # Moving horizontally.
                if self.matrix[nx][ny] == "[" and self.move_box_horiz_if_possible(
                    nx, ny, delta
                ):
                    self.update_cell(nx, ny, "@")
                    self.update_cell(cx, cy, ".")
                    return (nx, ny)
                elif self.matrix[nx][ny] == "]" and self.move_box_horiz_if_possible(
                    nx, ny - 1, delta
                ):
                    self.update_cell(nx, ny, "@")
                    self.update_cell(cx, cy, ".")
                ...
                if (
                    self.matrix[nx][ny] == "["
                    and self.move_box_vert_if_possible(nx, ny, delta)
                ) or (
                    self.matrix[nx][ny] == "]"
                    and self.move_box_vert_if_possible(nx, ny - 1, delta)
                ):
                    self.update_cell(nx, ny, "@")
                    self.update_cell(cx, cy, ".")
                    return (nx, ny)
    ...
    def move_boxes(self):
        ...
        for move_row in self.moves:
            for move in move_row:
                move_counter += 1
                self.footer_counter.config(
                    text="Moves: " + str(move_counter) + "/" + str(self.total_moves)
                )
